  ...
  const ProductPage = () => {
    ...
    // and hide it when the footer is in (or near) view
    useEffect(() => {
      const handleVisibility = () => {
        const el = purchaseBoxRef.current;
        if (!el) return setShowStickyBar(false);

        const purchaseRect = el.getBoundingClientRect();
        const purchaseVisible =
          purchaseRect.top < window.innerHeight && purchaseRect.bottom > 0;

        // Detect footer visibility (or proximity)
        const footer = document.querySelector("footer");
        let footerNearView = false;
        if (footer) {
          const footerRect = footer.getBoundingClientRect();
          const threshold = 24; // px tolerance so the bar hides slightly before overlapping
          footerNearView = footerRect.top <= window.innerHeight + threshold;
        }

        setShowStickyBar(!purchaseVisible && !footerNearView);
      };
                ...
                <div className="container px-4 sm:px-6 lg:px-8 mx-auto py-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1 flex items-center gap-3">
                    <img
                      ...
                      alt={product.name}
                      className="w-16 h-16 md:w-20 md:h-20 object-cover rounded-sm"
                    />
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        {selectedVariant?.name && (
                          <span className="text-[11px] md:text-xs px-2 py-0.5 rounded-full border border-gray-300 text-gray-700">
                            {selectedVariant.name}
                          </span>
                        )}
                  ...
                  {/* Middle: quantity selector (hidden on very small screens) */}
                  <div className="hidden sm:flex items-center border border-primary rounded-sm overflow-hidden bg-gray-100 w-32 h-10">
                    <button
                      onClick={decreaseQuantity}
                    ...
                    <div className="flex-1 text-center font-medium text-primary select-none">
                      {quantity}
                    </div>
                    <button
                      onClick={increaseQuantity}
                  ...
                  <button
                    onClick={handleAddToCart}
                    className="px-4 py-2 rounded-sm bg-primary text-white text-sm font-semibold hover:bg-primary/90"
                  >
                    Add to Cart
                  </button>
                </div>
